...
def get_unbalanced_sample_indices(target_classes: List, num_classes, n_per_digit=2, not_used_classes=[1, 2, 3, 4, 5]) -> List[int]:
    """Given `target_classes` randomly sample `n_per_digit` for each of the `num_classes` classes."""
    permed_indices = torch.randperm(len(target_classes[target_classes != not_used_classes]))
